...
# from src import pdf_mineru # No longer needed
...
class Pipeline:
    ...
    def parse_pdf_reports_parallel(self, chunk_size: int = 2, max_workers: int = 10):
        ...
        pdf_parser.debug_data_path = self.paths.debug_data_path
        ...
        print(f"PDF reports parsed and saved to {self.paths.reports_markdown_path}")

    def export_reports_to_markdown(self, file_name: str):
        """
        使用 PyMuPDF (fitz) 将本地 PDF 文件解析为纯文本，并保存为 Markdown 文件。
        :param file_name: PDF 文件名
        """
        pdf_path = self.paths.pdf_reports_dir / file_name
        if not pdf_path.exists():
            print(f"PDF 文件未在指定目录中找到: {pdf_path}")
            return

        try:
            print(f"开始使用 PyMuPDF 解析: {file_name}")
            # 打开 PDF 文件
            ...
            # 将提取的文本写入 Markdown 文件
            with open(target_path, "w", encoding="utf-8") as f:
                f.write(full_text)

            print(f"成功解析并将文本保存到: {target_path}")

        except Exception as e:
            print(f"使用 PyMuPDF 解析文件 {file_name} 时出错: {e}")
...
if __name__ == "__main__":
    ...
    print('4. 使用mineru将pdf转化为纯markdown文本')
    pdf_dir = pipeline.paths.pdf_reports_dir
    pdf_files = [f for f in os.listdir(pdf_dir) if f.lower().endswith('.pdf')]
    
    print(f"在目录 {pdf_dir} 中找到 {len(pdf_files)} 个PDF文件。")

    for file_name in pdf_files:
        try:
            print(f"\n{'='*20} 开始处理文件: {file_name} {'='*20}")
            pipeline.export_reports_to_markdown(file_name)
            print(f"{'='*20} 文件处理完成: {file_name} {'='*20}")
        except Exception as e:
            import traceback
            print(f"处理文件 {file_name} 时发生未捕获的严重错误: {e}")
            traceback.print_exc()
